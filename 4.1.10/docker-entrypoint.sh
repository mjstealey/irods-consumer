...
_usage() {
    ...
    echo "-i run_irods          initialize iRODS 4.1.10 consumer"
    echo "-x run_irods          use existing iRODS 4.1.10 consumer files"
    echo "-v                    verbose output"
    echo ""
    echo "Example:"
    echo "  $ docker run --rm mjstealey/irods-consumer:4.1.10 -h           # show help"
    echo "  $ docker run -d mjstealey/irods-consumer:4.1.10 -i run_irods   # init with default settings"
    echo ""
    exit 0
}
